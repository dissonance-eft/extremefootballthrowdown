...
**Intelligent Pathfinding (D3bot + NavMesh):**
- **Primary:** D3bot custom node graph (`gamemode/lib/d3bot/`) — hand-placed nodes per map via in-game editor (`d3bot_navmeshing` console command)
- **Node graph files:** Saved to `data/d3bot/navmesh/map/<mapname>.txt`
- **Fallback:** GMod NavMesh A* for maps without D3bot nodes
- **Last resort:** Direct LOS steering with TraceHull wall avoidance (ignores players to prevent "Circling" bug)
- **Pathfinding integration:** `sv_bot_pathfinding.lua` queries D3bot first → NavMesh second → direct steering
...
EFT bundles a custom fork of **D3bot** (`gamemode/lib/d3bot/`, 20 files) for bot pathfinding. This replaces GMod's unreliable built-in NavMesh with hand-placed node graphs per map.

| Property | Value |
|----------|-------|
| **Library path** | `gamemode/lib/d3bot/` |
| **Integration** | `sv_bot_pathfinding.lua` (priority: D3bot → NavMesh A* → direct LOS) |
| **Node graph storage** | `data/d3bot/navmesh/map/<mapname>.txt` |
| **Editor command** | `d3bot_navmeshing` (opens in-game VGUI node editor) |
| **Loaded by** | `init.lua` → `include("lib/d3bot/d3bot_init.lua")` |

**Setup per map:**
1. Load the map in singleplayer or on the server as superadmin
2. Run `d3bot_navmeshing` in console to open the node editor
3. Place nodes at key navigation points (corridors, ramps, platforms, jump pads)
4. Connect nodes with links (walk, jump, ladder)
5. Save — file auto-saves to `data/d3bot/navmesh/map/<mapname>.txt`
6. Bots will immediately use the new graph on next pathfinding query

**Without a node graph:** Bots fall back to NavMesh A* (if `.nav` file exists) or direct line-of-sight steering. Maps without any pathfinding data will have bots that run in straight lines and get stuck on geometry.
