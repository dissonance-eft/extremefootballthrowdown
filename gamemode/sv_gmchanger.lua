...
fretta_voting = CreateConVar( "fretta_voting", "1", { FCVAR_ARCHIVE } )
...
function GM:StartGamemodeVote()
	...
	if( !GAMEMODE.m_bVotingStarted ) then
		GAMEMANAGER:SetInGamemodeVote(true)
		GAMEMODE.m_bVotingStarted = true

		-- Use the new MapVote system instead of the old Fretta text-button vote.
		-- Since EFT is a single-gamemode setup, skip gamemode vote and go to map vote.
		if MapVote and MapVote.Start then
			MsgN("[EFT] Starting map vote via MapVote system...")
			MapVote.Start()
		else
			-- Fallback to old Fretta system if MapVote not loaded
			ErrorNoHalt("[EFT] MapVote not available, falling back to Fretta vote\n")
			if ( fretta_voting:GetBool() ) then
				if table.Count( g_PlayableGamemodes ) >= 2 then
					...
					GAMEMANAGER:SetVoteEndTime( CurTime() + fretta_votetime:GetFloat() )
				else
