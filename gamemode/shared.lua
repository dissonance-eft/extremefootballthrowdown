...
GM.FolderName = "extremefootballthrowdown"
...
if SERVER then
	CreateConVar("eft_gamelength", "15", FCVAR_REPLICATED + FCVAR_NOTIFY, "Time in minutes the map lasts for.")
	CreateConVar("eft_warmuplength", "30", FCVAR_REPLICATED + FCVAR_NOTIFY, "Time in seconds for the warmup phase to last.")
	CreateConVar("eft_overtime", "300", FCVAR_REPLICATED + FCVAR_NOTIFY, "Time in seconds for overtime to last.")
	CreateConVar("eft_overtime_scoreball", "15", FCVAR_REPLICATED + FCVAR_NOTIFY, "During these last seconds of overtime, the ball will be a score ball powerup and the round will never end until someone scores.")
...
GM.WarmUpLength = 30
GM.OvertimeTime = 300
GM.OvertimeScoreBall = 15
GM.Competitive = false
GM.VeryCompetitive = false
GM.ScoreLimit = 10
GM.Pity = 4
GM.VotingDelay = 5 -- Seconds before map vote starts after game ends

-- Change callbacks work on both realms (server changes, clients receive replicated value)
cvars.AddChangeCallback("eft_gamelength", function(cvar, oldvalue, newvalue)
	GAMEMODE.GameLength = tonumber(newvalue) or 15
	if GAMEMODE.GameLength < 0 then GAMEMODE.GameLength = -1 end
end)

cvars.AddChangeCallback("eft_warmuplength", function(cvar, oldvalue, newvalue)
	GAMEMODE.WarmUpLength = tonumber(newvalue) or 30
	if GAMEMODE.WarmUpLength < 0 then GAMEMODE.WarmUpLength = -1 end
end)

cvars.AddChangeCallback("eft_overtime", function(cvar, oldvalue, newvalue)
	GAMEMODE.OvertimeTime = tonumber(newvalue) or 300
	if GAMEMODE.OvertimeTime < 0 then GAMEMODE.OvertimeTime = -1 end
end)

cvars.AddChangeCallback("eft_overtime_scoreball", function(cvar, oldvalue, newvalue)
	GAMEMODE.OvertimeScoreBall = tonumber(newvalue) or 15
	if GAMEMODE.OvertimeScoreBall < 0 then GAMEMODE.OvertimeScoreBall = -1 end
end)
...
cvars.AddChangeCallback("eft_scorelimit", function(cvar, oldvalue, newvalue)
	GAMEMODE.ScoreLimit = tonumber(newvalue) or 10
	if GAMEMODE.ScoreLimit < 0 then GAMEMODE.ScoreLimit = -1 end
end)
...
function IncludePlayerClasses()
	...
	local files, _ = file.Find( "gamemodes/" .. folder .. "/gamemode/player_class/*.lua", "GAME" )
	for _, name in pairs(files or {}) do
		if name ~= "." and name ~= ".." then
			include("player_class/" .. name)
			if SERVER then AddCSLuaFile("player_class/" .. name) end
		end
	end
end
...
-- InRound is defined in round_controller.lua - do not duplicate here
...
function GM:GetRoundTime( iRound )
	-- Return remaining time until game ends, not a fixed value
	-- This prevents the round timer from resetting after each goal
	local timeLimit = self:GetTimeLimit()
	if timeLimit > 0 then
		local remaining = timeLimit - CurTime()
		return math.max(1, remaining) -- At least 1 second
	end
	return self.RoundLength -- Fallback to default if no time limit
end
...
function GM:GetTimeLimit()
	if GAMEMODE.GameLength > 0 then
		...
		if GAMEMODE.OvertimeTime > 0 and GAMEMODE:GetOvertime() then
			...
			if GAMEMODE.OvertimeScoreBall > 0 and CurTime() < time + 10 then -- Just in case the map breaks, give 2 minutes max for someone to touch the score ball.
				time = math_max(time, CurTime() + 0.1)
			end
		end
		
		-- Add any bonus time from goal celebrations (so celebration time doesn't count against game clock)
		time = time + GetGlobalFloat("BonusTime", 0)
...
-- Returns how much game time is left (in seconds)
function GM:GetGameTimeLeft()
	local limit = GAMEMODE:GetTimeLimit()
...
function GM:KeyPress(pl, key)
	...
	if key == IN_ATTACK then
		-- Punch is handled by weapon_eft:PrimaryAttack() which passes the correct duration.
		-- Do NOT handle it here — calling SetState without duration causes instant-end punch.
	elseif key == IN_ATTACK2 then
		if pl:CanMelee() then
			local vel = pl:GetVelocity()
			local dir = vel:GetNormalized()
			local speed = vel:Length() * dir:Dot(pl:GetForward())
			if speed >= 300 and CurTime() >= pl:GetLastChargeHit() + 0.4 and pl:GetCarry() ~= self:GetBall() then
